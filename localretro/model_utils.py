# code modified from https://github.com/SamLynnEvans/Transformer
...
def concat_graph(bg, atom_feats, bond_feats):
    edit_feats = []
    feat_dim = atom_feats.size(-1)
    sg = bg.remove_self_loop()
    sg.ndata['h'] = atom_feats
    ...
    for g in gs:
        e_feats = torch.cat((g.ndata['h'], g.edata['e']), dim = 0) 
        graph_feat = torch.sum(g.ndata['h'], dim = 0, keepdim = True)
        graph_feat = graph_feat.repeat(e_feats.size(0), 1)
        e_feats = torch.cat((e_feats, graph_feat), dim = 1)
        edit_feats.append(e_feats)
    edit_feats = pad_sequence(edit_feats, batch_first=True, padding_value= 0)
    return edit_feats
